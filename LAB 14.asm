...
LDI R3, char  ;R3 hold the ascii value of char
LEA R4, FONT_DATA
ADD R3,R3,R3
ADD R3,R3,R3
ADD R3,R3,R3
ADD R3,R3,R3 ; R3x16
ADD R4,R4,R3 ; R4 hold the address in FONT data, R4=start+R3x16(16 row each char)

rowloop  ADD	R5,	R1,	#-16
         BRz	DONE        ; determine whether it's end
         LDR	R2,	R4,	#0  ;load row value to R2
         ADD	R1,	R1,	#1  ;counter+1
         ADD    R4, R4, #1  ;next row

        column1     LD R5, mask1 ;load mask1 to R5
                    AND R6,R2,R5
                    BRz #3 
                    LDI R0, ones
                    out
...
char  .fill x5002
linefeed .fill x000A
mask1 .fill x8000   ;from 1000 0000 00000000 to 0000 0001 00000000, this will determine printout by bit
mask2 .fill x4000
mask3 .fill x2000
mask4 .fill x1000
mask5 .fill x0800
